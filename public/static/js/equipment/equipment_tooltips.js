...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showTooltip(slotElement, equipmentData, slotType) {
            ...
            Object.entries(equipmentData.stats).forEach(([stat, value]) => {
                if (value > 0) {
                    const statNames = {
                        ...
                        'crit_damage': 'Критический урон',       // ← добавить
                        'parry': 'Парирование',
                        'dodge': 'Уклонение',
                        'resist': 'Сопр. магии',
                        'block': 'Блок',
        ...
        // Таблица названий характеристик для камней концентрации
        const targetStatNames = {
            'hp': 'Здоровье', 'mp': 'Энергия',
            'hp_reg': 'Восст. Здоровья', 'mp_reg': 'Восст. Энергии',
            'attack_power': 'Сила атаки', 'attack_speed': 'Скорость атаки',
            'hit': 'Точность', 'crit': 'Шанс Крит. Урон',
            'crit_damage': 'Критический урон', 
            'dodge': 'Уклонение', 'parry': 'Парирование',
            'resist': 'Сопр. магии', 'armour': 'Броня',
            'spell_armour': 'Маг. Броня', 'block': 'Блок',
            'crit_damage_resistance': 'Сопр. Крит'
        };

        // Универсальный поиск имени камня по id во всех справочниках
        function resolveStoneName(stone) {
            let name = stone.id;
            ...
            if (typeof StonesData !== 'undefined') {
                const all = [
                    ...(StonesData.strength || []),
                    ...(StonesData.weapon || []),
                    ...(StonesData.concentration || [])
                ];
                const found = all.find(s => s.id === stone.id);
                if (found && found.name) name = found.name;
            }

            // Для камня концентрации добавляем цель
            if (stone.category === 'concentration' && stone.targetStat) {
                name = `${name} → ${targetStatNames[stone.targetStat] || stone.targetStat}`;
            }
            return name;
        }
        ...
        equipmentData.stones.forEach(stone => {
            const name = resolveStoneName(stone);
            if (!stoneGroups[name]) stoneGroups[name] = [];
            stoneGroups[name].push(stone.level);
        });
